...
namespace TiPEIS
{
    public partial class FormMOL : Form
    {
        ...
        Regex regFIO = new Regex(@"[/s/.a-zA-Zа-яА-Я]");
        ...
        private void toolStripButtonAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Поле ФИО должно содержать менее 50 символов");
                toolStripTextBox1.Text = "";
            }
            else if (!regFIO.IsMatch(toolStripTextBox1.Text))
            {
                MessageBox.Show("Поле ФИО должно содержать только текст");
                toolStripTextBox1.Text = "";
            }
            else
            {

                string txtSQLQuery = "insert into MOL (idMOL, Name) values (" +
